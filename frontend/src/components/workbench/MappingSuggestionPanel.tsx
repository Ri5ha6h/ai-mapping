...
export function MappingSuggestionPanel({
  suggestions,
  usedAi,
  statusText,
  providerErrors,
  ...
  return (
    <WorkbenchCard
      kicker="Script hints"
      title="Likely field links"
      icon={<BrainCircuit size={18} />}
    >
      <div className="flex flex-wrap gap-2">
        <Badge variant="outline">{statusText}</Badge>
        <Badge variant="secondary">
          {usedAi ? "OpenRouter" : "Deterministic"}
        </Badge>
      ...
      <div className="grid gap-2">
        {suggestions.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No field hints generated yet.
          </p>
        ) : (
          suggestions.map((suggestion) => (
            <div
              className="grid gap-2 rounded-lg border bg-muted/25 px-3 py-2"
              key={suggestion.id}
            >
