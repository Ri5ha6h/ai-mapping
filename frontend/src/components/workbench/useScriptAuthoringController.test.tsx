...
vi.mock("@/lib/effect/api_effects", async () => {
  ...
  return {
    getMappingCapabilitiesEffect: () => Effect.succeed({ ai_mapping_available: true }),
    suggestMappingsEffect: (...args: unknown[]) => suggestMappingsEffect(...args),
    generateScriptDraftEffect: (...args: unknown[]) => generateScriptDraftEffect(...args),
  }
})
...
describe("useScriptAuthoringController", () => {
  it("generates field hints and tracks AI/local provider status", async () => {
    ...
    suggestMappingsEffect.mockReturnValue(
      Effect.succeed({
        suggestions: [{ source_path: "$.id", target_path: "$.id", confidence: 0.98, rationale: "match" }],
        used_ai: false,
    ...
    await waitFor(() => expect(screen.getByTestId("ai-available").textContent).toBe("yes"))
    act(() => screen.getByRole("button", { name: "ai mode" }).click())
    await act(async () => screen.getByRole("button", { name: "hints" }).click())

    expect(suggestMappingsEffect).toHaveBeenCalledWith(baseInputs.sourceSchema, baseInputs.targetSchema, true)
    expect(screen.getByTestId("status").textContent).toBe("AI failed, local used")
    expect(screen.getByTestId("providers").textContent).toBe("provider unavailable")
  ...
  it("generates scripts and clears stale run results when script changes", async () => {
    ...
    generateScriptDraftEffect.mockReturnValue(
      Effect.succeed({
        mapping_spec: { engine: "script_js", script_version: 1, script: "function transform() { return { id: 1 }; }" },
    ...
    expect(screen.getByTestId("explanation").textContent).toBe("Generated from local hints.")
    expect(screen.getByTestId("unresolved").textContent).toBe("$.missing")
    expect(clearRunResults).toHaveBeenCalledTimes(2)
  })
})

function AuthoringProbe({ clearRunResults = vi.fn() }: { clearRunResults?: () => void }) {
  ...
  return (
    <div>
      <span data-testid="ai-available">{controller.aiMappingAvailable ? "yes" : "no"}</span>
      ...
      <span data-testid="unresolved">{controller.unresolvedTargetPaths.join(",")}</span>
      ...
      <button onClick={() => controller.setScript("function transform(source) { return source; }")}>edit</button>
    </div>
  )
}
