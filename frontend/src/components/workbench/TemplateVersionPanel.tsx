    ...
    >
      <div className="grid gap-3">
        ...
        <div
          ...
          aria-label="Template save options"
        >
          <div className="rounded-lg border bg-muted/25 p-3">
            <strong>Save template</strong>
            <span className="block text-sm text-muted-foreground">
              Updates the selected template name, notes, and current workspace
              snapshot.
            </span>
          </div>
          <div className="rounded-lg border bg-muted/25 p-3">
            ...
            <span className="block text-sm text-muted-foreground">
              Adds a numbered snapshot while keeping older versions loadable.
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            disabled={!canSave || busy || templateName.trim().length === 0}
          >
            <Save size={15} />
            Save template
          </Button>
          ...
          <Button
            ...
            onClick={() =>
              selectedTemplate
                ? onDeleteTemplate(selectedTemplate.template_id)
                : undefined
            }
        ...
        {selectedTemplate?.is_seeded ? (
          <StatusAlert
            title="Example"
            description="Includes source and target samples."
          />
        ) : null}
        ...
        {selectedTemplate ? (
              ...
              .map((version) => (
                <div
                  className={[
                    ...
                    version.version === selectedTemplate.active_version
                      ? "border-primary bg-secondary/40"
                      : "",
                  ].join(" ")}
                  key={`${selectedTemplate.template_id}-${version.version}`}
                >
                  <div className="grid min-w-0 gap-1">
                    ...
                    <span className="text-xs text-muted-foreground">
                      ...
                      {version.field_validation_rules.length} field rules
                    </span>
                  </div>
                  <Button
                    type="button"
          ...
          <p className="text-sm text-muted-foreground">
            Save a script or select an existing template.
          </p>
        )}
        ...
        <div className="grid gap-3 rounded-lg border bg-card p-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge variant="outline">
              {deletedTemplates.length} archived templates
            </Badge>
          </div>
          {deletedTemplates.length === 0 ? (
            ...
            <div className="grid gap-2">
              {deletedTemplates.map((template) => (
                <div
                  className="flex min-w-0 flex-col gap-3 rounded-lg border bg-muted/25 p-3 sm:flex-row sm:items-center sm:justify-between"
                  key={template.template_id}
                >
                  <div className="grid min-w-0 gap-1">
                    <strong>{template.name}</strong>
                    <span className="text-sm text-muted-foreground">
                      v{template.active_version} · {template.versions.length}{" "}
                      version(s) · archived template
                    </span>
                  </div>
                  <Button
                    type="button"
